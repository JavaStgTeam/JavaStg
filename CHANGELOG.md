...
## 20226-03-01
- 弃用日志文件,改用git提交记录替代日志文件

## 2026-02-21

### 修改
- 修改了 `ObjectPoolManager` 类，添加了 `getOrCreatePool()` 和 `getRegisteredTypes()` 方法，支持动态创建对象池
- 修改了 `Obj.create()` 方法，确保对象池已初始化，修复了类型转换问题，支持不同类型的参数
- 优化了对象池管理逻辑，确保所有创建的对象都被正确跟踪和计数

### 修复
- 修复了敌人没有被算作对象池内容的问题，确保关卡中的敌人正确使用对象池
- 修复了对象池对象数统计错误的问题，确保窗口标题中的obj数量正确显示

### 测试
- 验证了修改后的对象池系统能够正确跟踪和计数敌人、子弹等游戏对象
- 测试了窗口标题中的obj数量显示，确认能够实时反映对象池中的对象总数
